...
impl Compiler {
    ...
    fn compile_expr(&mut self, expr: &Expr) -> Result<(), String> {
        match expr {
            ...
            Expr::ArrayAssign { array, index, op, value } => {
                ...
                // Get the base variable name from the array expression
                let var_name = match array.as_ref() {
                ...
                if *op != AssignOp::Assign {
                    ...
                    // This requires more stack manipulation
                ...
                if index.is_some() {
                    ...
                    // For append, pop the null key and use ArrayAppend
                    self.emit(Opcode::Swap);  // value, null, array -> value, array, null
                    self.emit(Opcode::Pop);   // value, array
            ...
            Expr::EnumCase { enum_name, case_name } => {
                // Push enum case as a value
                let enum_idx = self.intern_string(enum_name.clone());
                let case_idx = self.intern_string(case_name.clone());
                // For now, push a placeholder - need proper enum handling
                self.emit(Opcode::PushString(enum_idx));
                self.emit(Opcode::PushString(case_idx));
            ...
            Expr::CallableCall { callable, args } => {
                ...
                // For now, we need a special opcode or builtin for callable invocation
                // This is complex - skip for now
                return Err("CallableCall not yet fully implemented".to_string());
            }
            ...
            Expr::NewAnonymousClass { constructor_args, parent: _, interfaces: _, traits: _, properties, methods } => {
            ...
            Expr::CloneWith { object, modifications } => {
                ...
                for modification in modifications {
                    ...
                    self.emit(Opcode::StoreProperty(prop_idx));
                    // Pop the returned object from StoreProperty
                    self.emit(Opcode::Pop);
                }
        ...
        for prop in properties {
            ...
            if prop.is_static {
                ...
                compiled_class.static_properties.insert(
                    prop.name.clone(),
                    crate::interpreter::Value::Null, // Default will be set at runtime
                );
            }
    ...
    fn compile_enum(
        ...
        // Store enum cases
        for case in cases {
            // For backed enums, we'd need to evaluate the value expression
            // For now, store None (pure enum behavior)
            compiled_enum.cases.insert(case.name.clone(), None);
        }
